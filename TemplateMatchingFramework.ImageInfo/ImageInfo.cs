...
public class ImageInfo(Vector4[] values) : IComparer
{
    ...
    public Guid ToGuid()
    {
        if (Values.Length == 0) return Guid.Empty;
        List<byte> bytes = [];
        int skipCount = Values.Length / 4;
        for (int i = 0; i < Values.Length; i += skipCount)
        {
            bytes.Add((byte)Values[i].X);
            bytes.Add((byte)Values[i].Y);
            bytes.Add((byte)Values[i].Z);
            bytes.Add((byte)Values[i].W);
        }
